...
The optional large-volume line reports estimated snapshot memory only to avoid
allocating a 512 x 512 x 256 volume in constrained CI containers.
"""
...
from pkdqc.core.regions import RegionReviewState, build_region_index, delete_region_checked
...
def main():
    ...
    print(f"component-index flat-index memory: {component_bytes / 1024 / 1024:.2f} MiB")
    large_bytes = 512 * 512 * 256 * np.dtype(np.uint16).itemsize
    print(f"optional 512x512x256 uint16 snapshot estimate: {large_bytes / 1024 / 1024:.2f} MiB (not allocated)")
